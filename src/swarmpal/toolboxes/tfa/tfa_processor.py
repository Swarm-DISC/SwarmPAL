...
class TfaInput(ExternalData):
    ...
    def flag_cleaning(self, varname = None, flagname = None):
        """Set flagged values to NaN"""
        # if no inputs are given, default to the class variables
        if varname is None: varname = self.VARNAME
        if flagname is None: flagname = self.FLAGNAME
        
        inds_to_remove = self.xarray[flagname] > self.MAX_FLAG_VAL
    ...
    def calculate_magnitude(self, input_varname = None, output_varname = None):
        ...
        if "NEC" in self.VARNAME:
            # if no inputs are given, default to the class variables
            if input_varname is None: input_varname = self.VARNAME
            if output_varname is None: output_varname = "B"

            X = np.sqrt(np.sum(self.xarray[input_varname]**2, 1))
            self.append_array(output_varname, X.data, dims=("Timestamp"))
        else:
            print("TfaInput: Will not calculate magnitude! Input field is not in vector form.")
    ...
    def subtract_chaos(self, field_varname=None, model_varname=None):
        ...
        if self.REMOVE_CHAOS:
            # if no inputs are given, default to the class variables
            if field_varname is None: field_varname = self.VARNAME
            if model_varname is None: model_varname = self.VARNAME + "_Model"
                
            B_res = self.xarray[field_varname].data - self.xarray[model_varname].data
            self.xarray[field_varname].data = B_res        
        else:
            print("TfaInput: Cannot subtract CHAOS if the 'remove_chaos' parameter has not been set to True")
    
    def convert_to_mfa(self):
        """Convert field from NEC to MFA coordinates"""
        # only run for NEC variables
        if "NEC" in self.VARNAME:
            B_MFA = tfalib.mfa(self.xarray["B_NEC"].data, 
                               self.xarray["B_NEC_Model"].data)
    ...
    def __init__(self, varname="B_NEC", remove_chaos = False, 
                 sampling_time = "PT1S", *args, **kwargs):
        ...
        if "HR" in self.COLLECTION:
            self.DEFAULTS["pad_times"] = [dt.timedelta(minutes=5), 
                                     dt.timedelta(minutes=5)]
        else:
            self.DEFAULTS["pad_times"] = [dt.timedelta(hours=3), 
        ...
        if "EXPT" in self.COLLECTION:
            ...
            v = self.VARNAME[0:2]
            E_XYZ = np.vstack((self.xarray[v+"x"], 
                               self.xarray[v+"y"],
                               self.xarray[v+"z"])).T
...
class TfaProcessor:
    ...
    def __init__(self, input_data, active_variable, params=None):
        ...
        t_old_sec = (t_old - t_old[0]) / np.timedelta64(1, 's') # converts diff to seconds
        
        # apply "constant_candence"
        new_t_sec, new_X = tfalib.constant_cadence(
            t_old_sec, X, 1/self.input_data.SAMPLING_TIME, 
            interp = False)[0:2]
        new_t = t_old[0] + new_t_sec * np.timedelta64(1, 's')
        
        # insert new values to xarray Dataset
        self.input_data.xarray = self.input_data.xarray.assign_coords(
                                  {"time": new_t})
    ...
    def create_segment_index(self):
        ...
        old_t = (self.input_data.xarray["Timestamp"].data - \
                self.input_data.xarray["Timestamp"].data[0]).astype(np.float64)
        new_t = (self.input_data.xarray["time"].data - \
                        self.input_data.xarray["time"].data[0]).astype(np.float64)
        ...
        si[(self.input_data.xarray["time"].data < tlims[0]) |
                      (self.input_data.xarray["time"].data > tlims[-1])] = np.NaN
        si = si - np.nanmin(si)
    ...
    def get_segment_inds_and_lims(self, segment):
        if segment is None:
            ...
            if segment > self.Max_Segment: segment = self.Max_Segment
    ...
    def plotX(self, full=False, segment=None):
        ...
        # plt.figure()
        plt.plot(self.input_data.xarray["time"].data[inds], 
    ...
    def image(self, full=False, segment=None, cbar_lims = None, log=True):
        ...
        # plt.subplot(D,1,i+1)
        if log:
            plt.contourf(self.input_data.xarray["time"].data[inds], freqs, 
    ...
    def plotAUX(self, full=False, segment=None):
        ...
        # plt.figure()
        # fig,ax = plt.subplots()
        plt.plot(self.input_data.xarray["Timestamp"].data, 
                 self.input_data.xarray["QDLat"].data, "-b")
        ax = plt.gca()
        ax.set_ylabel("QDLat (deg)")
        ax.set_ylim([-90, 90])
        ax.set_yticks([-90, -45, 0, 45, 90])
        ax2=ax.twinx()
        ax2.plot(self.input_data.xarray["Timestamp"].data, 
                 self.input_data.xarray["MLT"].data, "-r")
    ...
    def wave_index(self):
        if "W" in self.input_data.xarray:
            ...
            print("wave_index(): No wavelet array 'W' found! Must apply the Wavelet function first!")
    
    
    def interp_nans(self):
        i = np.arange(len(self.input_data.xarray["X"]))
        nonNaNinds = np.where(~np.isnan(self.input_data.xarray["X"].data))
        self.input_data.xarray["X"].data = np.interp(i, i[nonNaNinds], 
                                self.input_data.xarray["X"].data[nonNaNinds])
    ...
    def wave_detection(self, threshold = 0):
        if "W" in self.input_data.xarray:
            ...
            self.input_data.xarray["W"].data[:, np.where(maxInds == nFreqs-1)] = np.NaN
            ...
            if self.input_data.COLLECTION[0:2] == "SW":
                ...
                request.set_products(measurements=["Bubble_Probability"], 
                                     sampling_step="PT1S")
            ...
            print("wave_detection(): No wavelet array 'W' found! Must apply the Wavelet function first!")
...
class Filtering(TFA_Process):
    ...
    def __init__(self, params=None):
        if params is None:
            self.params = {"Sampling_Rate": 1, "Cutoff": 5 / 1000}
        else:
            self.params = params
...
class Wavelet(TFA_Process):
    ...
    def apply(self, target):
        ...
        # insert new values to xarray Dataset 
        target.input_data.xarray = target.input_data.xarray.assign_coords(
                                  {"scale": s})
