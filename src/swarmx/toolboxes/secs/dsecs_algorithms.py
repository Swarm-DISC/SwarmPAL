...
def _legPol_n1(n,x,ind=False):
    ...
    if n > 2:
        for j in range(2,n):
            Pa[:,j] = 2*x*Pa[:,j-1] - Pa[:,j-2] + (x*Pa[:,j-1] - Pa[:,j-2])/(j-1)
    if (ind or n==1):
        return -1*Pa[:,n]
    else:
        return -1*Pa
...
def _legPol(n,x,ind=False):
    ...
    for i in range(2,n+1):
        P[:,i] = 2*x*P[:,i-1] - P[:,i-2] - (x*P[:,i-1]-P[:,i-2])/i
...
def SECS_1D_DivFree_vector(latV,latSECS,ri):
    ...
    theta_secs = (90. - latSECS)*np.pi/180
    theta_v = (90. - latV)*np.pi/180
...
def SECS_2D_DivFree_vector(thetaV,phiV,thetaSECS,phiSECS,radius,limitangle):
    ...
    for n in range(Nv):
        ...
        dummy = ComFac * 1/np.tan(0.5 * limitangle[~distant])**2 /(1 + CosThetaPrime[~distant])
        matVtheta[n, ~distant] = dummy * sinC[~distant]
        matVphi[n, ~distant] = dummy * cosC[~distant]
...
def SECS_2D_DivFree_magnetic(thetaB,phiB,thetaSECS,phiSECS,rb,rsecs):
    ...
    for n in range(Nb):
        ...
        #See Eq. (A5) and Fig. 14 of Vanhamäki et al.(2003)
        CosThetaPrime = np.cos(thetaB[n])*np.cos(thetaSECS) + np.sin(thetaB[n])*np.sin(thetaSECS)*np.cos(phiSECS - phiB[n])
        ...
        cosC = (np.cos(thetaSECS)-np.cos(thetaB[n])*CosThetaPrime) / (np.sin(thetaB[n]*Sin2ThetaPrime))
        ...
        if rb[n] < rsecs:
            ...
            auxHorizontal = -factor[n] * ((ratio[n] -CosThetaPrime)/auxroot + CosThetaPrime)
        elif rb[n] > rsecs:
            auxVertical = ratio[n]
...
def SECS_2D_CurlFree_antisym_vector(thetaV,phiV,thetaSECS,phiSECS,radius,limitangle):
    ...
    Vp,Vt = SECS_2D_DivFree_vector(thetaV,phiV,thetaSECS,phiSECS,radius,limitangle)
    Vp = -Vp

    #Anti-SECS
    [aVp,aVt] = SECS_2D_DivFree_vector(thetaV, phiV, np.pi - thetaSECS, phiSECS, radius, limitangle)
    aVp = -aVp
...
def SECS_2D_CurlFree_antisym_magnetic(thetaB,phiB,thetaSECS,phiSECS,rb,rsecs,limitangle):
    ...
    bt = -bt
    abp,abt = SECS_2D_DivFree_magnetic(thetaB,phiB,athetaSECS,phiSECS,aux,rsecs)[1:]
    abt = -abt
    aux = aux/rb
    ...
    for n in range(Nsecs):
        ...
        smallr = 1.1*rsecs*limitangle[n] # Optimal choice probably depends on altitude, but ignore it here
        ...
        bx,by,bz = secs_2d_curlFree_antisym_lineintegral(thetaB,phiB,rb,thetaSECS[n],phiSECS[n],rsecs,smallr)

        #add to the magnetic field of the horizontal current
        matBradial[:,n] = bx * np.sin(thetaB) * np.cos(phiB) + by *np.sin(thetaB)*np.sin(phiB) + bz*np.cos(thetaB)
        matBtheta[:,n] = matBtheta[:,n] + bx * np.cos(thetaB) * np.cos(phiB) + by * np.cos(thetaB) * np.sin(phiB) - bz * np.sin(thetaB)
...
def secs_2d_curlFree_antisym_lineintegral(thetaB,phiB,thetaSECS,phiSECS,rb,rsecs,smallr):
    """_summary_
    ...
    if 0:
        ...
        x = np.pi/2 - thetaSECS
        s = L * np.abs( np.sin(x) * np.sqrt(3*np.sin(x)**2 - 1 )
        + 1/np.sqrt(3)*np.arcsinh(np.sqrt(3)*np.sin(x)))
        Nint = np.ceil(s/step)
    
    dt = (2 * thetaSECS -np.pi)/Nint # negative if SECS pole at the northern hemisphere
    t = (np.pi - thetaSECS) + (np.arange(1,Nint+1,dtype=np.double) - .5 )*dt
    ...
    root = np.where(root<smallr*smallr*smallr,smallr,root)#np.maximum(root,smallr**3)
