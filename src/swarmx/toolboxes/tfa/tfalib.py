...
def constant_cadence(t_obs, x_obs, sampling_rate, interp = False):
    ...
    if len(x_obs.shape) == 2:
        ...
        if x_obs.shape[0] != N and x_obs.shape[1] != N:
            sys.exit("constant_cadence: ERROR: `x_obs` must have the same length as `t_obs`")
...
def moving_mean_and_stdev(x, window_size, unbiased_std = True):
    ...
    if len(original_shape) > 2:
        sys.exit("moving_mean_and_stdev: ERROR: `x` argument must be 1- or 2-dimensional array")
    ...
    if window_size >= N:
        sys.exit("moving_mean_and_stdev: ERROR: `window_size` cannot be equal or larger than the length of the data series in `x`")
...
def moving_q25_and_q75(x, window_size):
    ...
    D = pd.DataFrame(x)
    moving_window = D.rolling(window_size, min_periods = int(window_size/2)) # accept windows of minimum W/2 valid points
    moving_q25 = moving_window.quantile(.25, interpolation='linear').to_numpy()
    moving_q75 = moving_window.quantile(.75, interpolation='linear').to_numpy()
...
def outliers(x, window_size, method = 'iqr', multiplier = np.NaN):
    ...
    if method == 'normal':
        ...
        sys.exit("outliers: ERROR: `method` not recognized! Choose between 'normal' or 'iqr'")
...
def filter(x, sampling_rate, cutoff):
    ...
    sos = signal.cheby2(7, 10, cutoff, btype='lowpass', analog=False, output='sos', fs=sampling_rate)
...
def morlet_wave(N=600, scale=1, dx=0.01, omega=6.203607835633639, roll=True, 
                norm=True):
    """
...
def wavelet_transform(x, dx, wavelet_function, minScale, maxScale, dj=0.1):
    """
    ...
    N = len(x)
    M = np.log2(maxScale/minScale)+dj
    scales = minScale * np.power(2, np.arange(0, M, dj))
    N_scales = len(scales)
...
def wavelet_normalize(wave_sq_matrix, scales, dx, dj, wavelet_norm_factor):
    ...
    normalized_wave_sq_matrix: (M,N) Array like
        ...
        transform
    """  
    return (2*dx*dj / wavelet_norm_factor) * wave_sq_matrix / np.reshape(scales, (-1,1))
